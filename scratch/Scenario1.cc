...
main (int argc, char *argv[])
{
    ...
    { // *** 5G ***
        ...
        // BWP0, FDD-DL
        nrHelper->GetGnbPhy (enbNetDevs.Get (0), 0)->SetAttribute ("Numerology", UintegerValue (numerology));
        nrHelper->GetGnbPhy (enbNetDevs.Get (0), 0)->SetAttribute ("Pattern", StringValue ("DL|DL|DL|DL|DL|DL|DL|DL|DL|DL|"));
        nrHelper->GetGnbPhy (enbNetDevs.Get (0), 0)->SetTxPower (30);

        // BWP1, FDD-UL
        nrHelper->GetGnbPhy (enbNetDevs.Get (0), 2)->SetAttribute ("Numerology", UintegerValue (numerology));
        nrHelper->GetGnbPhy (enbNetDevs.Get (0), 2)->SetAttribute ("Pattern", StringValue ("UL|UL|UL|UL|UL|UL|UL|UL|UL|UL|"));
        nrHelper->GetGnbPhy (enbNetDevs.Get (0), 2)->SetTxPower (0);

        // Link the two FDD BWP:
        nrHelper->GetBwpManagerGnb (enbNetDevs.Get (0))->SetOutputLink (2, 0);
    ...
    { // *** 4G ***
        ...
        for (uint16_t i = 0; i < numberOfueNodes; i++)
        {
            lteHelper->Attach (ueNetDevs.Get(i), enbNetDevs.Get(0));
